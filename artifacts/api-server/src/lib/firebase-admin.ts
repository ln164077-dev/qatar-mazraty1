...
if (firebaseConfigured) {
  try {
    ...
    console.warn('[Firebase] Notifications disabled: invalid Firebase credentials', error);
  }
} else {
  console.warn('[Firebase] Notifications disabled: Firebase credentials are not configured');
}
...
export async function sendPushNotification(
  ...
  if (!messaging) {
    return {
      success: false,
      error: "إشعارات Firebase غير مفعّلة على الخادم",
