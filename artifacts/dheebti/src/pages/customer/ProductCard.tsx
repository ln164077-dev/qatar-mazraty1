...
function money(value: number) {
  return `${value.toFixed(0)} ريال قطري`;
}
    ...
    return (
      ...
      >
        <div className="relative aspect-square overflow-hidden rounded-xl bg-muted">
          ...
          />
          <span className={`absolute right-2 top-2 rounded-full px-2 py-0.5 text-[8px] font-bold ${product.id === 32 ? 'bg-red-600 text-white' : 'bg-accent text-secondary'}`}>{product.id === 32 ? 'قارب على الانتهاء' : 'طازج'}</span>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-square overflow-hidden rounded-t-2xl bg-muted">
        ...
        />
        <span className={`absolute right-2 top-2 rounded-full px-2 py-1 text-[8px] font-bold ${product.id === 32 ? 'bg-red-600 text-white' : 'bg-accent text-secondary'}`}>{product.id === 32 ? 'قارب على الانتهاء' : 'طازج'}</span>
      </div>
